    ...
    <main className="mx-auto max-w-5xl px-4 py-10 space-y-12">
      ...
      <section>
        ...
        <div className="mt-5 flex items-center gap-3">
          {open ? (
            <a className="rounded-2xl bg-[#4A1F3D] px-5 py-3 text-white" href={wa("full", 1)} target="_blank">
              Order Now
            </a>
          ) : (
            <a className="rounded-2xl bg-[#4A1F3D] px-5 py-3 text-white" href="#notify">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <input className="w-16 rounded-md border px-2 py-1" type="number" min={1} value={qtyH}
                onChange={(e) => setQtyH(parseInt(e.target.value || "1"))} />
              <a className="ml-auto rounded-xl bg-[#4A1F3D] px-4 py-2 text-white" href={wa("half", qtyH)} target="_blank">
                Order Half
              </a>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <div className="mt-3 flex items-center gap-2">
              <label className="text-sm">Qty</label>
              <input className="w-16 rounded-md border px-2 py-1" type="number" min={1} value={qtyF}
                onChange={(e) => setQtyF(parseInt(e.target.value || "1"))} />
              <a className="ml-auto rounded-xl bg-[#4A1F3D] px-4 py-2 text-white" href={wa("full", qtyF)} target="_blank">
                Order Full
              </a>
            </div>
          </div>
      ...
      <footer className="border-t pt-6 text-sm opacity-70">
        ...
        <div className="mt-1">
          <a className="underline" href="/privacy">Privacy</a> • <a className="underline" href="/terms">T&Cs</a> • Registered home food business (Tufnell Park)
        </div>
      </footer>
    </main>
  );
